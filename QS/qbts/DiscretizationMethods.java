 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
final public class DiscretizationMethods {
 
	
	//	 Metodos de discretización  - Discrtcretization Methods
	public static final String[] KNOWN_METHODS_NAMES = new String[] { "CAIM",
			"Ameva", "CUM", "EFI", "EWI", "none" };
    ...
    //	 Medidas de Similitud  - Similarity Measures
	public static final String[] KNOWN_SIMILARITY_METHODS = new String[] {
			"QSI", "IntervalKernel" , "IKMulti", "DTW", "Euclidean"};
	...
	static final	 public void compute_CAIM_Ameva(List D, int metodo, int numClases,
			...
			do {
				...
				case CAIM: //CAIM
					...
					break;
				case Ameva: // ChiSplit
					tmpCriterion = nextMaxAmeva(D, numCortes, numClases, acumulados,
							mejor);
					break;
		//		case CUM: // CUM
		//			break;
	 ...
	 */
	static final private float nextMaxAmeva(List D, int numCortes, int numClases,
			int[][] acu, int[] mejor) {
		...
		for (int co = 1; co < numCortes; co++) {
			...
			if (vale) {
				...
				//añade el corte actual a una lista auxiliar
				...
				for (int icorte = 0; icorte < numInt; icorte++)
					for (int cla = 0; cla < numClases; cla++) {
						valorChi += Math.pow(matrizCA[cla][icorte], 2)
								/ (matrizCA[cla][numInt] * matrizCA[numClases][icorte]);
					}
	 ...
	 * other.
	 */
	static final public void Ordena_Vectores(double[] v1, String sOrder, int iLow1,
			int iHigh1, double[] v2) {
		//Dimension variables
		int iLow2, iHigh2;
		...
		while (iLow2 < iHigh2) {
			if (sOrder.equals("A")) {
				...
				while ((v1[iLow2] < vItem1) && (iLow2 < iHigh1))
					iLow2++;
				//'Find the last item that is less than the mid-point item
				while ((v1[iHigh2] > vItem1) && (iHigh2 > iLow1))
					iHigh2--;
			} else {
		...
		if (iHigh2 > iLow1)
			Ordena_Vectores(v1, sOrder, iLow1, iHigh2, v2);
		//'Recurse to sort the upper half of the extremes
		if (iLow2 < iHigh1)
			Ordena_Vectores(v1, sOrder, iLow2, iHigh1, v2);
	}
	...
	static final public void Ordena_Vectores(int[] v1, String sOrder, int iLow1,
			int iHigh1, int[] v2) {
		//Dimension variables
		int iLow2, iHigh2;
		int  vItem1, vItem2;
...
//	**********************************************************************************************
		//**********************************************************************************************
	static final	public void Compute_EWI(List D, int intervalos) {
	 ...
	 */
	static final public double QSIndex(double[] pcVx, double[] pcVy) {
	 ...
	 * @return
	 */
	static final public double EuclideanSeries(double[] pfVx, double[] pfVy) {
		double valor=0;
		int sizex;
		...
		for (int i=0;i<sizex;i++)
			valor+=Math.pow(pfVx[i]-pfVy[i],2); 
	 ...
	 * @return
	 */
	static final public double DTW(double[] pfVx, double[] pfVy) {
		   double min;
		   int i,j;
...
//		 creación dinámica
		   ...
		   // EN Orden  1= Superior 2 = Izquierda   3 = Diagonal
		   ...
		   for (i=2;i<=sizex;i++)
			   for(j=2;j<=sizey;j++)
			   {
				   ...
				   if (D[i-1][j]<min)
				   {
					   min= D[i-1][j];
					   dir=1;
					   // CORRECION hasta ahora para encontrar el mínimo si el izquierdo era menor
					   // que el valor de la diagonal no se comparaba con el superior.
					   if (D[i][j-1]<min)
					   {
						   min= D[i][j-1];
						   dir=2;
	 ...
	 */
	static final public double Kernel_Intervalar(double[] dVx, double[] dVy, double[] discre,double paraSimil) {
	...
	//**********************************************************************************************
	//**********************************************************************************************
static final	public  void Compute_CUM(List D, int numIntervalos, int numClases,
			int numCortes, int[][] acumulados) {

	// numIntervalos buscados
	// numCortes candidatos= los extremos mas los intermedios del resto
...
static private int find_root(double datos[][],int num_datos, int inicial,double valor){
for (int i=inicial; i<= num_datos; i++)
	if (datos[i][1]>valor){
		return i; 
	}
return -1;
		...
		//**********************************************************************************************
		//**********************************************************************************************
	static final	public  void Compute_EFI(List D, int numIntervalos, int numClases,
				int numCortes, int[][] acumulados) {

			int[] acum = new int[acumulados[0].length];
			for (int i = 0; i < acum.length; i++) {
				for (int c = 0; c < numClases; c++)
					acum[i] += acumulados[c][i];
				if (i > 0)
					acum[i] += acum[i - 1];
			}
			long total = acum[acum.length - 1];

			//long ele = total / numIntervalos;
			int pos_act = 0;
			...
			for (int vuelta = 1; vuelta < numIntervalos; vuelta++) {
				...
				long valor = (int) t;
				//este es el valor de corte y tengo que buscar el
				// punto intermedio que acumule
				do {
					pos_act++;
